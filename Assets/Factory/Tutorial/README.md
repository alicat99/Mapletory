...
`SampleScene`을 실행하고 1스테이지에 입장한다. 새 진행 데이터에서는 상단 튜토리얼이 자동으로 시작한다. 우클릭 드래그, 휠 줌, 컨베이어 선택, 회전 가능한 건물 선택 후 R, 컨베이어 설치, X, 실제 철거를 순서대로 수행한다. 각 행동 직후에만 다음 단계로 이동하고 마지막 건물 설명을 확인하면 다시보기 버튼이 나타나야 한다.
