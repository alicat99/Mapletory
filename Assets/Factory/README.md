...
Edit Mode 자동 테스트는 Test Runner에서 `Maptory.Factory.Tests` 어셈블리를 실행한다. 테스트는 등각 화면 방향 매핑, 직선 배치와 끝 이미지, 대각선 거부, 기존 방향 덮어쓰기, 다중 출력의 A 이미지, 다중 입력의 무변형, 출력 라운드로빈 분배, 입력 반대 방향으로의 역류 방지를 검증한다.
