...
Play Mode가 시작되면 스테이지 선택 화면에 3개 스테이지가 표시되고 1스테이지만 처음부터 입장 가능한지 확인한다. 잠긴 스테이지는 해금 비용과 구매 팝업을 표시하며, 메소가 충분할 때만 영구 해금 후 입장한다. 1스테이지에 입장한 뒤 다음을 확인한다.
...
46. 디버그 `해금` 탭은 스테이지·사냥터별 메소 비용과 사냥터 필요 몬스터 재료·수량을 수정한다. `변경사항 저장 후 처음부터 실행`은 이 설정과 몬스터·업그레이드 밸런스만 별도 설정 세이브에 유지하고 메소·생산량·업그레이드·스테이지·사냥터 진행을 초기화해 새 게임 상태로 재시작한다.
...
| `PortalSystem.cs` | 7종 몬스터 공급 항목, 2×2 포탈과 잠금 선택 검증, 저장 가능한 메소·생산량·합연산/곱연산 업그레이드 상태 소유 |
...
| `PortalSelectionPanel.cs` | 현재 스테이지 사냥터 목록, 잠금 행, 메소·재료 조건 구매 팝업과 포탈 품목 선택 처리 |
...
메소, 몬스터별 누적·사용 가능 생산량, 두 업그레이드 레벨과 스테이지·사냥터 해금은 진행 세이브에 저장된다. 디버그에서 명시적으로 저장한 몬스터 밸런스·업그레이드 비용·스테이지와 사냥터 조건은 진행과 분리된 설정 세이브에 저장된다. 컨베이어·건물 배치와 디버그 맵 편집은 아직 저장하지 않으므로 스테이지 재입장 시 초기 맵으로 시작한다.
