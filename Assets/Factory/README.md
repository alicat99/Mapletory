...
36. 선택 후 월드 툴팁은 `<아이템> x.x/분 | x.x메소/분`으로 바뀐다. 품목별 공급량은 최근 10초 안의 입력 시각 간 평균 간격을 역산한다. 입력이 4개 이상이면 계산값을 즉시 적용하고, 3개 이하이면 6초 반감기 EMA로 직전 값과 섞으므로 공급이 끊기면 자연스럽게 0으로 감소한다. 같은 품목을 선택한 모든 포탈은 `PortalEconomy`의 같은 측정 창을 조회하여 합산된 분당 공급량과 메소 생산량을 공유한다.
...
포탈 좌표는 2×2 발자국의 좌하단 앵커다. `InputPorts`는 컨베이어 위치, 포탈 내부 도착 위치와 필수 진행 방향을 함께 제공한다. 선택 품목을 공급하면 네트워크 공용 `PortalEconomy`가 메소와 품목별 최근 10초 입력 시각을 갱신한다. `Update`는 측정 창을 진행하고 입력이 드문 품목의 표시값을 감쇠한다.
...
| `PortalSystem.cs` | 7종 몬스터 공급 선택 항목, 2×2 포탈 상태와 입력 포트, 메소 지갑 및 품목별 최근 10초 공유 속도 통계 소유 |
...
| `MesoHud.cs` | 화면 좌측 상단 `<누적량> 메소` 표시와 공용 포탈 경제 측정 시계 진행 |
...
| `Tests/EditMode/PortalTests.cs` | 포탈 2×2 점유·8개 입력·운송 소비·품목 필터·메소·최근 10초 공유 속도·희소 입력 감쇠·Sprite 검증 |
...
현재 저장은 구현되어 있지 않다. 에르다 결과는 별도 월드 개체가 아니라 기존 아이템 운송 상태를 사용하므로 막힌 끝 X 규칙과 라운드로빈 합류·분배를 그대로 따른다. 포탈의 최근 입력 시각과 표시 속도는 런타임 세션 통계이며 저장되지 않는다.
