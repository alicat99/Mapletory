...
공장 출력 위치가 다음 공장의 입력 포트와 바로 맞닿으면 `FactoryItemTransport`가 컨베이어 대신 해당 소비자를 목적지로 예약한다. 아이템은 생산 공장의 출력 포트에서 다음 공장의 입력 포트로 이동하며, 목적 공장이 받을 수 있을 때만 생산 측 재료를 소비한다.
...
| `MesoHud.cs` | 화면 좌측 상단 `<누적량> 메소` 표시와 공용 포탈 경제 표본 시간 진행 |
