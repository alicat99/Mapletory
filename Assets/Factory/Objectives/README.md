...
스테이지 오른쪽에서 현재 목표를 확인한다. 원재료 생산, 첫 몬스터 제작, 포탈 공급, 업그레이드, 사냥터 해금, 다음 몬스터 제작을 진행하면 실제 생산·경제·해금 이벤트에 따라 한 단계씩 이동해야 한다. `레시피 보기`를 누르면 목표 대상이 선택된 E 도감이 열린다. 재입장 후 같은 단계가 유지되는지 확인한다.
...
| 파일 | 책임 |
|---|---|
| `FactoryObjectiveProgressData.cs` | 현재 목표 단계 저장 |
