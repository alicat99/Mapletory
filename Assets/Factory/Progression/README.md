...
3. 스테이지 선택 화면에 머물거나 다른 스테이지에 입장한 동안에도 같은 실행 세션의 기존 공장 포탈 생산량과 메소가 계속 증가하는지 확인한다.
...
7. Play Mode를 종료하고 다시 시작했을 때 공장 배치, 스테이지·사냥터 해금, 메소와 몬스터 진행도가 모두 초기화되는지 확인한다.

Edit Mode의 `ProgressionTests`는 기본 해금, 스테이지·사냥터 구매 원자성, 중복 구매, 부족 조건의 부분 차감 방지, 잠긴 몬스터 직접 선택 차단, 설정 직렬화, 선택 화면의 UI 입력 시스템과 실행 세션 초기화를 검증한다.
...
기본 사냥터 해금 비용은 1스테이지가 `0 / 50 / 300`, 2스테이지가 `0 / 1,000 / 3,000 / 5,000 / 10,000` 메소다. 디버그에서 변경한 비용은 현재 실행 세션에만 적용된다.
...
| `FactorySaveService.cs` | Scene 전환용 진행 데이터, 스테이지별 공장 배치와 디버그 설정을 실행 중 메모리에 복사 |
| `FactoryProgression.cs` | 해금 상태, 구매 검증·원자적 소비와 현재 스테이지 세션 소유 |
...
`FactoryGame`은 현재 실행 세션의 설정을 적용한 `PortalEconomy`와 `FactoryProgression`을 먼저 만든 뒤, 스테이지 선택 화면 또는 해당 스테이지의 공장 런타임을 조립한다. 컨베이어, 건물 방향, 레시피, 포탈 선택, 이동 중 아이템과 기계 내부 대기 재료는 Scene 전환 동안만 메모리에 유지된다. 현재 보이지 않는 공장은 `FactoryHeadlessRuntime`이 같은 운송 규칙으로 계속 갱신하므로 같은 실행 세션의 스테이지 선택 화면과 다른 스테이지에서도 메소 생산이 유지된다. `PlayerPrefs`나 파일 저장은 사용하지 않으며 앱을 종료하거나 Play Mode를 다시 시작하면 모든 상태가 초기화된다.
