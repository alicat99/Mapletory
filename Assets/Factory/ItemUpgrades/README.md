...
기본 메소 비용은 달팽이부터 순서대로 `50 / 100 / 400 / 500 / 1,500 / 4,500 / 7,500 / 15,000`, 메소 비용 계수는 `1.25`다. 생산량 기본 비용은 모두 `100`, 생산량 비용 계수는 `5`다. 기본 가치는 `1 / 2 / 3 / 5 / 7 / 10 / 20 / 30`, 메소 레벨당 증가량은 각각 기본 가치의 10%, 생산 레벨당 배율은 모두 `1.5배`다.
...
업그레이드 진행도와 디버그에서 바꾼 비용·효과·계수는 현재 실행 세션에만 유지되며 앱을 다시 시작하면 기본값으로 초기화된다.
