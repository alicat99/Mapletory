...
7. `변경사항 저장 후 처음부터 실행`을 누르면 현재 실행 중에는 잔디·원재료 맵 편집, 몬스터 가치, 업그레이드 비용·계수, 스테이지·사냥터 비용이 유지되고 메소·업그레이드·해금·공장 건설 진행은 초기화된 스테이지 선택 화면으로 재시작하는지 확인한다.
8. 저장한 스테이지에 재입장해 잔디와 원재료 배치가 복원되는지 확인하고, 다시 `F2`를 눌러 패널을 닫는다.
9. 디버그 값을 다시 변경한 뒤 저장 후 재시작 버튼을 누르지 않고 우측 상단 `돌아가기`로 스테이지를 나갔다가 재입장한다. 밸런스·해금 비용·맵 설정이 그대로 유지되어야 한다.
10. 공장 플레이 중 `F3`을 누르면 현재 디버그·맵 설정은 실행 세션에 유지되고, 플레이 진행 데이터가 초기화된 뒤 빌드의 첫 Scene인 스테이지 선택 화면으로 이동해야 한다.
11. 앱 또는 Play Mode를 종료하고 다시 시작하면 디버그 설정과 진행 데이터가 모두 기본값으로 초기화되어야 한다.
...
`F3`은 디버그 패널의 `변경사항 저장 후 처음부터 실행` 버튼과 같은 동작을 한다. 현재 실행 세션의 설정은 유지하고 진행 상태와 스테이지별 공장 상태를 삭제한 뒤 빌드 인덱스 0을 로드한다.

## 3. 코드 구조와 책임

| 파일 | 책임 |
|---|---|
| `FactoryDebugPanel.cs` | `F2` 패널, `F3` 진행 초기화, 맵·몬스터·업그레이드·해금 탭, 숫자 입력, 전체 설정 저장과 새 게임 재시작 |
| `FactoryDebugMapEditor.cs` | 잔디·원재료·제거·아이템 브러시 입력과 스테이지별 맵 설정 캡처 |

디버그 도구는 별도의 복제 상태를 만들지 않고 `ExtractionNetwork`, `PortalEconomy`, `FactoryProgression`, `FactoryItemTransport` 등 정식 시스템의 공개 명령만 호출한다. 명시적 재시작 버튼은 밸런스·해금·현재 스테이지 맵 설정을 실행 중 메모리에 남기고 플레이 진행 상태를 삭제한다. 우측 상단 `돌아가기`도 같은 설정을 세션에 유지하지만 진행 데이터는 유지한다. 앱을 종료하면 디버그 설정을 포함한 모든 세션 상태가 사라진다.
