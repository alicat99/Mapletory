...
기능 폴더는 필요에 따라 `Editor`와 `Tests` 하위 폴더를 가질 수 있다. 이는 Unity 컴파일 및 테스트 어셈블리 경계를 위한 예외이며, 기능에 속한 파일을 기능 루트 밖으로 분산하지 않는다.
...
깨끗한 Scene 또는 명시된 검증 Scene에서 전체 실험 환경을 구성하는 방법을 설명한다. 필요한 `GameObject`, `Component`, prefab, `ScriptableObject`, Inspector 설정값, 실행 순서, 입력 방법과 정상 동작 시 관찰되는 결과를 구체적으로 기록한다. 자동 테스트가 있다면 테스트 커버리지와 실행 방법도 포함한다.
...
공용 코드는 최소 두 기능에서 실제로 사용될 때만 상위 기반 기능으로 추출한다. 포괄적인 `Common`, `Utils` 또는 `Managers` 폴더를 편의상 만들지 않는다. 기능 간 순환 참조를 만들지 않으며, 의미 있는 기능 경계는 assembly definition으로 명시한다.
